...
bool LoadPngFromAssetManager(int target, const std::string& path) {
  ...
  static struct JNIData {
    ...
    constexpr char kHelperClassName[] =
        "com/pxy/lark/cloudxrdemo/JniInterface";
...
void
GetPoseRawFromAnchor(const ArAnchor &ar_anchor, ArSession *ar_session, float *out_pose_raw) {
  if (out_pose_raw == nullptr) {
    LOGE("util::GetPoseRawFromAnchor out_pose_raw is null.");
    return;
  }
  util::ScopedArPose pose(ar_session);
  ArAnchor_getPose(ar_session, &ar_anchor, pose.GetArPose());
  ArPose_getPoseRaw(ar_session, pose.GetArPose(), out_pose_raw);
}
